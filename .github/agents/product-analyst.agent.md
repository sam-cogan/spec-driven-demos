...
tools: ['search', 'codebase', 'fetch', 'githubRepo', 'editFiles', 'mcp_github_github_issue_read']
handoffs:
...
- **Acceptance criteria must be testable.** No "the system should be fast" — use measurable, observable statements (Given/When/Then).
- **Explicit scope.** Always list non-goals to prevent scope creep.
- **Ask, don't assume.** If the issue is ambiguous, list your questions in an "Open Questions" section rather than guessing.
